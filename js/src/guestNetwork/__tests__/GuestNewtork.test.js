...
describe("<GuestNetwork/>", () => {
  ...
  describe("enabled DHCP and QoS", () => {
    ...
    beforeEach(() => {
      ...
      changeStart = (value) =>
        fireEvent.change(getByLabelText(guestNetworkContainer, "DHCP start"), {
          target: { value },
        });
    });
    ...
    it("should validate router's IP", async () => {
      ...
      await wait(() => {
        expect(
          getByText(guestNetworkContainer, "This is not a valid IPv4 address.")
        ).toBeDefined();
      });
    });
    ...
    it("should validate network mask", async () => {
      ...
      await wait(() => {
        expect(
          getByText(guestNetworkContainer, "This is not a valid network mask.")
        ).toBeDefined();
      });
    });
    ...
    it("should validate lease time", async () => {
      ...
      await wait(() => {
        expect(
          getByText(guestNetworkContainer, "Minimum lease time is 1 hour.")
        ).toBeDefined();
      });
    });
