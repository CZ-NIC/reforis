 ...
 */
function isDHCPStartWithinNetwork(
    networkAddress,
    prefixLength,
    DHCPStartAddress
) {
...
function isDHCPStartReserved(
    ...
    if (
        [broadcastAddress.toString(), networkAddress.toString()].includes(
            DHCPStartAddress.toString()
        )
    ) {
        return true;
    }
...
export default function validateDHCP(
    ...
    if (!errors.start) {
        const prefixLength = ipaddr.IPv4.parse(
            networkMask
        ...
        if (
            !isDHCPStartWithinNetwork(networkAddress, prefixLength, DHCPStart)
        ) {
            errors.start = _("Address is outside current network.");
        } else if (
            isDHCPStartReserved(networkAddress, broadcastAddress, DHCPStart)
        ) {
            errors.start = _("Address is already reserved for other purposes.");
        } else if (!errors.limit) {
            // There are no more important errors so far.
            if (
                !isMaxLeasesValid(
                    networkAddress,
                    prefixLength,
                ...
                DHCPStart.toString() === routerIPString &&
                dhcpSettings.limit === 1
            ) {
                errors.limit = _(
