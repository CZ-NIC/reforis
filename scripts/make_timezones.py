...
def generate_timezones():
    # l18n can't find country code of this regions for some reason...
    # This countrycodes are gotten from http://clock.zone
    no_country_code = {
        'America/Santa Isabel': 'MX',
        'Asia/Rangoon': 'MM',
        'Pacific/Johnston': 'US',
    }
    ...
    res = {}
    for luci_tz, zone_info in luci_TZ:
        region = luci_tz.split('/')[0]
        country_code = l18n.utils.get_country_code_from_tz(luci_tz.replace(" ", "_"))
        if country_code is None:
            country_code = no_country_code[luci_tz]
        city = str(l18n.tz_cities[luci_tz.replace(" ", "_")])

        res.setdefault(region, {}).setdefault(country_code, {})[city] = (luci_tz, zone_info)
    return res
...
def generate_countries():
    ...
    countries = {}
    timezones = generate_timezones()
    for _, country_codes in timezones.items():
        for country in country_codes:
            countries[country] = str(l18n.territories[country])
    return countries
...
def main():
    ...
    with codecs.open(output_file, 'w', 'utf-8') as outfile:
        outfile.write(f'''
        export const TIMEZONES = {json.dumps(generate_timezones(), ensure_ascii=False)};
        export const COUNTRIES = {json.dumps(generate_countries(), ensure_ascii=False)};
        ''')
